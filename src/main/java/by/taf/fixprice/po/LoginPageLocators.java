package by.taf.fixprice.po;

public class LoginPageLocators {
    public static final String EMAIL_TAB_LOCATOR = "//button[@class='button small'][text() = 'По email']";
    public static final String PHONE_TAB_LOCATOR = "//button[@class='button small active'][text() = 'По номеру телефона']";
    public static final String EMAIL_INPUT_LOCATOR = "//input[@data-test='input'][@type='email']";
    public static final String PHONE_INPUT_LOCATOR = "//input[@data-test='input'][@type='text']";
    public static final String PASSWORD_INPUT_LOCATOR = "//input[@data-test='input'][@type='password']";
    public static final String ENTER_LOCATOR = "//button[@class='button enter-button normal']";
}
